...
use super::config::{parse_ssh_config, SshHostConfig};
...
impl SessionManager {
    ...
    pub async fn connect_with_config(&self, host_alias: &str, config: &SshHostConfig) -> Result<()> {
        ...
        match session.userauth_agent(user).await {
            Ok(_) => {
                if session.authenticated() {
                    ...
                    eprintln!("[DEBUG] SSH agent authentication returned OK but session not authenticated");
                }
            }
            Err(e) => {
                if debug {
        ...
        if !authenticated {
            if let Some(ref identity_file) = config.identity_file {
                ...
                if identity_file.exists() {
                    match session.userauth_pubkey_file(user, None, identity_file, None).await {
                        Ok(_) => {
                            if session.authenticated() {
                                authenticated = true;
                                if debug {
                        ...
                        match session.userauth_pubkey_file(user, None, &path, None).await {
                            Ok(_) => {
                                if session.authenticated() {
                                    authenticated = true;
                                    if debug {
                                        eprintln!("[DEBUG] Key file authentication successful: {}", path.display());
                                    }
                                    break;
                                } else if debug {
                                    eprintln!("[DEBUG] Key file authentication returned OK but session not authenticated: {}", path.display());
                                }
                            }
                            Err(e) => {
                                if debug {
                                    eprintln!("[DEBUG] Key file authentication failed: {} - {}", path.display(), e);
                                }
                            }
                        }
                    } else if debug {
        ...
        if !authenticated {
            anyhow::bail!("SSH key authentication failed. No valid keys found or agent not available. Check that SSH agent is running or keys exist in ~/.ssh/");
        }
    ...
    pub async fn connect_direct(&self, host_alias: &str, user: &str, host: &str, port: Option<u16>) -> Result<()> {
    ...
    pub async fn connect_with_password(&self, host_alias: &str, user: &str, host: &str, password: &str, port: Option<u16>) -> Result<()> {
    ...
    pub async fn execute_command(&self, host_alias: &str, command: &str) -> Result<crate::ssh::channel::CommandOutput> {
        let mut sessions = self.sessions.lock().await;
        let state = sessions.get_mut(host_alias)
    ...
    #[allow(dead_code)]
    pub async fn execute_command_streaming(&self, host_alias: &str, command: &str) -> Result<String> {
        let mut sessions = self.sessions.lock().await;
        let state = sessions.get_mut(host_alias)
            .ok_or_else(|| anyhow::anyhow!("Not connected to host '{}'", host_alias))?;
